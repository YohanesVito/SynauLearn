-- ============================================
-- Insert Base Course - All-in-One Script
-- ============================================
-- This script inserts everything in one transaction
-- No need to manually copy IDs between steps!
-- ============================================
...
DECLARE
  ...
  v_category_id UUID;
BEGIN
  -- Get or check category ID
  SELECT id INTO v_category_id
  FROM categories
  WHERE slug = 'blockchain-fundamentals'
  LIMIT 1;

  -- If category not found, set to NULL (course will have no category)
  -- Optionally: Remove the IF block to make category required

  RAISE NOTICE '=== Starting Base Course Insertion ===';
  ...
  VALUES (
    ...
    'id',
    'Basic',
    v_category_id, -- Will be NULL if category doesn't exist
    3,
    NOW()
  )
  ...
  VALUES (gen_random_uuid(), v_course_id, 'Introduction to Base', 1, 10, NOW())
  RETURNING id INTO v_lesson_1_id;

  RAISE NOTICE 'Lesson 1 created with ID: %', v_lesson_1_id;

  -- Tier 2
  INSERT INTO lessons (id, course_id, title, lesson_number, total_cards, created_at)
  VALUES (gen_random_uuid(), v_course_id, 'Ekosistem & Base Tools', 2, 10, NOW())
  RETURNING id INTO v_lesson_2_id;

  RAISE NOTICE 'Lesson 2 created with ID: %', v_lesson_2_id;

  -- Tier 3
  INSERT INTO lessons (id, course_id, title, lesson_number, total_cards, created_at)
  VALUES (gen_random_uuid(), v_course_id, 'Konsep Lanjutan tentang Base', 3, 10, NOW())
  RETURNING id INTO v_lesson_3_id;

  RAISE NOTICE 'Lesson 3 created with ID: %', v_lesson_3_id;

  -- ============================================
  -- Step 3: Insert Cards for Tier 1 (10 cards)
  -- ============================================

  RAISE NOTICE 'Inserting cards for Tier 1...';
  ...
  -- ============================================
  -- Step 4: Insert Cards for Tier 2 (10 cards)
  -- ============================================

  RAISE NOTICE 'Inserting cards for Tier 2...';
  ...
  -- ============================================
  -- Step 5: Insert Cards for Tier 3 (10 cards)
  -- ============================================

  RAISE NOTICE 'Inserting cards for Tier 3...';
  ...
  RAISE NOTICE '=== Base Course Insertion Complete! ===';
  RAISE NOTICE 'Course ID: %', v_course_id;
  RAISE NOTICE 'Tier 1 ID: %', v_lesson_1_id;
  RAISE NOTICE 'Tier 2 ID: %', v_lesson_2_id;
  RAISE NOTICE 'Tier 3 ID: %', v_lesson_3_id;
  RAISE NOTICE 'Total Cards Inserted: 30';
